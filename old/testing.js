...
function readDirectories() {
	...
	var directories = process.argv.slice(3, process.argv.length);
	
	for (var i = 0; i < directories.length; i ++) { readDirectory(directories[i]); }

	function readDirectory(directory) {
		fs.readdirSync(directory, function(err, dirFiles) {
			var dir = process.argv[3].replace('/', '') + '/';
			if (err) { throw err; }
			else {
				for ( var f = 0; f < dirFiles.length; f ++ ) {
					files.push(dir.concat(dirFiles[f]));
					console.log(dirFiles);
				}
				//if(i===directories.length-1){readFiles();}
			}
		});
	}
}
 ...
 */
function checkFlag() {
	if ( process.argv[2] === "-r" ) { readDirectories(); }
	else { 
